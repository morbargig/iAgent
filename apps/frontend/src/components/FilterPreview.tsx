...
export function FilterPreview({
  ...
  const formatDateFilter = () => {
    ...
    if (
      ...
      filterConfig.dateFilter.customRange
    ) {
      return `${filterConfig.dateFilter.customRange.amount} ${filterConfig.dateFilter.customRange.type} ago`;
    } else if (
      filterConfig.dateFilter.type === "picker" &&
      filterConfig.dateFilter.dateRange
    ) {
      try {
        ...
        if (typeof filterConfig.dateFilter.dateRange.start === "string") {
          ...
          filterConfig.dateFilter.dateRange.start &&
          typeof filterConfig.dateFilter.dateRange.start === "object"
        ) {
        ...
        if (typeof filterConfig.dateFilter.dateRange.end === "string") {
          ...
          filterConfig.dateFilter.dateRange.end &&
          typeof filterConfig.dateFilter.dateRange.end === "object"
        ) {
          // Handle dayjs objects or objects with toDate method
    ...
    >
      <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
        ...
        <List sx={{ p: 0 }}>
          ...
          {filterConfig.toolConfigurations &&
            Object.keys(filterConfig.toolConfigurations).length > 0 && (
              <ListItem sx={{ px: 0, py: 1 }}>
                <ListItemIcon sx={{ minWidth: "36px" }}>
                  <ConfigIcon
                ...
                <ListItemText
                  ...
                  secondary={
                    <Box sx={{ mt: 0.5 }}>
                      {Object.entries(filterConfig.toolConfigurations).map(
                        ([toolId, config]) => (
